/* 
	...
	- Problem: CF ROUND 1454 E - Number of Simple Paths
	- Problem link: https://codeforces.com/contest/1454/problem/E
*/
...
int Ares_KN() // main
{
	...
	while (nTest--) {
		...
		// find (u, v) edge to remove, graph now forms a tree with n-1 edges
		pii removed_edge = {-1, -1};
		FOR(u, 1, n)
			if (!visit[u]) {
				par[u] = -1;
				dfs_bridge(u);
		...
		//cerr << removed_edge.fi << ' ' << removed_edge.se << endl;
		int root = removed_edge.fi;
		int V = removed_edge.se;
		REP(i, g[root].size()) {
			if (g[root][i] == V) {
				FOR(j, i, g[root].size() - 2) 
					g[root][j] = g[root][j + 1];
				break;
			}
		}
		...
		ll res = 1LL * n * (n - 1) / 2;
		ll add = 0;
		// add edges that must pass (u, v) edge
		add += 1LL * (n - sub[V] ) * sub[V];
		int u = par[V];
		int child = sub[V];
		while (u != root) {
			int from = sub[u] - child;
			int to = n - sub[u];
			add += 1LL * from * to;
			child = sub[u];
			u = par[u];
