/* 
	...
	- Problem: CF ROUND 1873 - G - ABBC or BACB
...
int getA(int l, int r) {
	return !l ? cntA[r] : cntA[r] - cntA[l - 1];
}

int cost(int l, int r) {
	if (l >= r) return 0;
	// case B... and ...B 
	if (s[l] == 'B' || s[r] == 'B') return getA(l, r);
	
	// case A..A
	FOR(i, l, r)
		if (s[i] == 'B') {
			int le = getA(l, i) + cost(i + 1, r);
			int ri = cost(i, r); // since s[i]=B -> guarantees O(1)
			return max(le, ri);
		}
    return 0;
}
