/* 
	...
	- Problem: CF ROUND 1873 - E - Building an Aquarium
...
int Ares_KN() // main
{
	...
	while (nTest--) {
		...
		auto fill_tank = [&](int h) {
			auto it = lower_bound(begin(a), end(a), h);
			if (it != begin(a)) {
				--it;
				return 1LL * h * (it - begin(a) + 1) - sum[it - begin(a)];
			}
			else {
				return 0LL;
			}
		...
		// binary search the height
		ll lo = 1, hi = 2e9;
		do {
			ll mid = (lo + hi) >> 1;
			if (fill_tank(mid) > volume) {
				hi = mid - 1;
			}
			else {
				lo = mid;
