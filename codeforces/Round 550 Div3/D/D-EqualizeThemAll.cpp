/* 
	...
	- Problem: CF ROUND 1144 D - Equalize Them All
...
int Ares_KN() // main
{
	...
	REP(i, n)
	{
		cin >> a[i];
		++cnt[a[i]];
		if (cnt[a[i]] > cur.fi) cur = {cnt[a[i]], a[i]};
	}
	...
	vector<int> pos;
	REP(i, n) if (a[i] == cur.se) pos.push_back(i);
	REP(i, pos.size())
	{
		int j = pos[i] - 1;
		while (j >= 0 && a[j] != cur.se)
		{
			// operator
			if (a[j] > cur.se) printf("2 %d %d\n", j + 1, j + 2);
			if (a[j] < cur.se) printf("1 %d %d\n", j + 1, j + 2);
			--j;
		}
	}
	int j = pos.back() + 1;
	while (j < n && a[j] != cur.se)
	{
		// operator
		if (a[j] > cur.se) printf("2 %d %d\n", j + 1, j);
		if (a[j] < cur.se) printf("1 %d %d\n", j + 1, j);
