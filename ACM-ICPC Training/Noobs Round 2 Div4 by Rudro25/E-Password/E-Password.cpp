/* 
	...
	- Problem: Noobs Round #2 Div.4 - E - Password
...
ll get(int len, int digit) {
	if (digit == 1 || len == 1) return 1;
	if (dp[len][digit] != -1) return dp[len][digit];
	ll res = (get(len, digit - 1) + get(len - 1, digit)) % MOD;
	return dp[len][digit] = res, res;
}

ll get_total(int len, int digit) {
	ll res = 0;
	FOR(i, 1, digit) {
		res += get(len, i);
		res %= MOD;
	...
	while (nTest--) {
		...
		REP(i, s.size()) {
			if (s[i] == '-') {
				...
				if (cnt > 0) res = (res * get_total(cnt, s[i] - '0' - last + 1)) % MOD;
				cnt = 0;
				last = s[i] - '0';
			}
		}
		...
		if (cnt > 0) res = (res * get_total(cnt, 9 - last + 1)) % MOD;
		printf("%lld\n", res);
	}
