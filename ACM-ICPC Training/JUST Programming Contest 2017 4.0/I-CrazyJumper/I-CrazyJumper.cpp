/* 
	...
	- Problem: ACM JUST Programming Contest 2017 4.0 - I - The Crazy Jumper
	- Problem link: https://codeforces.com/gym/101532/problem/I
*/
...
int Ares_KN() // main
{
	...
	while (nTest--)
	{
		...
		while (!Q.empty())
		{
			...
			if (u == n - 1) break;
			Q.pop();
			if (f[u + 1] == -1) Q.push(u + 1), f[u + 1] = f[u] + 1;
			if (go[u] != -1)
			{
				int v = go[u];
				if (f[v] == -1) Q.push(v), f[v] = f[u] + 1;
			}
		}
		printf("%d\n", f[n - 1]);
	}
