/* 
	...
	- Problem: ACM JUST Programming Contest 2017 4.0 - D - Counting Test
	- Problem link: https://codeforces.com/gym/101532/problem/D
*/
...
{
	if (pos == -1) return 0;
	if (pos < len) return f[pos][c];

	int p = pos / len;
	return p * f[len - 1][c] + f[pos % len][c];
}
...
int Ares_KN() // main
{
	...
	while (nTest--)
	{
		...
		REP(i, s.size())
		{
			if (i > 0) FOR(c, 'a', 'z') f[i][c] = f[i - 1][c];
			++f[i][s[i]];
		}
