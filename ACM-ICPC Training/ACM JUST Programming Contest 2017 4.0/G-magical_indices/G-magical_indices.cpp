/* 
	...
	- Problem: ACM JUST Programming Contest 2017 4.0 - G - Magical Indices
	- Problem link: https://codeforces.com/gym/101532/problem/G
*/
...
int Ares_KN() // main
{
	...
	while (nTest--)
	{
		...
		r[n - 1] = a[n - 1];
		FORD(i, n - 2, 0) r[i] = min(r[i + 1], a[i]);
		FOR(i, 1, n - 2) res += l[i] <= a[i] && r[i] >= a[i];
		printf("%d\n", res);
	}
	return 0;
}
