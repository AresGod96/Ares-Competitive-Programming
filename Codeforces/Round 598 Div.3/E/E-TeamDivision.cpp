/* 
	...
	- Problem: CF ROUND 1256 E - Yet Another Division Into Teams
	- Problem link: https://codeforces.com/contest/1256/problem/E
*/
...
int Ares_KN() // main
{
	...
	int team = 1;
	dp[2] = {a[2].fi - a[0].fi, 1};
	trace[0] = trace[1] = trace[2] = -1;
	pair<ll, int> T = {0, -1};
	FOR(i, 3, n - 1)
	{
		dp[i] = {a[i].fi - a[0].fi, 1};
		trace[i] = -1;
		if (i < 5) continue;
		if (T.fi >= dp[i - 3].fi - a[i - 2].fi)
			T = {dp[i - 3].fi - a[i - 2].fi, i - 3};
			
		//cerr << i << ' ' << dp[i] << ' ' << T.fi << endl;
		//REP(j, i - 2)
		//	dp[i] = min(dp[i], a[i].fi + dp[j] - a[j + 1].fi);
		
		if (dp[i].fi >= a[i].fi + T.fi)
		{
			dp[i] = {a[i].fi + T.fi, dp[T.se].se + 1};
			trace[i] = T.se;
		}
	}
	...
	printf("%lld %d\n", dp[n - 1].fi, dp[n - 1].se);
	int cur = n - 1;
	int cur_team = dp[n - 1].se;
	int last = n - 1;
	vector<int> res(n);
	do
	{
		int from = trace[cur];
		FOR(i, from + 1, last) res[a[i].se] = cur_team;
		--cur_team;
		last = from;
		cur = trace[cur];
	} while (cur_team && cur != -1);
