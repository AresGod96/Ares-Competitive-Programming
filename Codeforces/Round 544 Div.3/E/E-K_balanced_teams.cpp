/* 
	...
	- Problem: CF ROUND 1133 E - K Balanced Teams
...
int Ares_KN() // main
{
	...
	{
		int first = i;
		FORD(j, i - 1, 0) if (a[j] < a[i] - 5) break; else first = j;
		prev[i] = first;
	}
	...
	FOR(i, 1, n - 1)
		FOR(j, 1, min(i + 1, k))
		{
			...
			int p = prev[i];
			if (p == 0) dp[i][j] = max(dp[i][j], i + 1);
			else dp[i][j] = max(dp[i][j], dp[p - 1][j - 1] + (i - prev[i] + 1));
			res = max(res, dp[i][j]);
		}
	printf("%d\n", res);
