/* 
	...
	- Problem: CF ROUND 1133 B - Preparation for International Women's Day
	- Problem link: https://codeforces.com/contest/1133/problem/B
*/
...
int Ares_KN() // main
{
	...
	REP(i, n)
	{
		...
		if (x % k == tmp)
		{
			if (cnt[tmp] > 1) res += 2, cnt[tmp] -= 2;
			continue;
		}
		else if (cnt[x % k] && cnt[tmp])
		{
			res += 2;
			--cnt[x % k], --cnt[tmp];
		}
