...
int Ares_KN() // main
{
	...
	while (nTest--) {
		...
		if (cnt >= 3) {
			...
			// middle segment contains two max values at border
			int first = -1, last = -1;
			REP(i, n)
				if (a[i] == max_val) {
					if (first == -1)
			...
			int le = pre[first - 1], ri = suf[last + 1];
			int mid = 2e9;
			FOR(i, first, last) 
				mid = min(mid, a[i]);
			while (first > 1 && a[first - 1] > mid) {
				--first;
				le = pre[first - 1];
			}
			while (last < n - 2 && a[last + 1] > mid) {
				++last;
				ri = suf[last + 1];
			}

			if (le == ri && le == mid) {
				printf("YES\n%d %d %d\n", first, last - first + 1, n - last - 1);
				continue;
			}

			bool gotans = false;
			while (1) {
				int val_le = -1, val_ri = -1;
				// take left
				if (first > 1) {
					int l = first - 1;
					val_le = min({pre[l - 1], mid, a[l], suf[last + 1]});
				}
				// take right
				if (last < n - 2) {
					int r = last + 1;
					val_ri = min({pre[first - 1], mid, a[r], suf[r + 1]});
				}
				if (val_le == -1 && val_ri == -1) break;
				if (val_le > val_ri) {
					--first;
					mid = min(mid, a[first]);
				}
				...
				if (pre[first - 1] == suf[last + 1] && pre[first - 1] == mid) {
					gotans = true;
					printf("YES\n%d %d %d\n", first, last - first + 1, n - last - 1);
					break;
				}
