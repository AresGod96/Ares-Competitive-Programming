/* 
	...
	- Problem: CF ROUND 1272 D - Remove One Element
...
int Ares_KN() // main
{
	...
	good.emplace_back(from, to);
	//for (auto x: good) cerr << x.fi << ' ' << x.se << endl;
	...
	FOR(i, 1, n - 2)
	{
		int x = i - 1;
		int y = i + 1;
		if (a[y] > a[x] && in[y] != in[x])
		{
			int add_y = good[in[y]].se - good[in[y]].fi + 1;
			int add_x = good[in[x]].se - good[in[x]].fi + 1;
			int tmp = add_x + add_y;
			if (in[i] == in[i - 1]) --tmp;
			if (in[i] == in[i + 1]) --tmp;
			res = max(res, tmp);
