/* 
	...
	- Problem: CF ROUND 1133 F2 - Spanning Tree with One Fixed Degree
	- Problem link: https://codeforces.com/contest/1133/problem/F2
*/
...
int deg[MAXN]; // degree in original graph
int chose[MAXN];
int par[MAXN];
...
void dfs(int u) {
	for (auto v: g[u])
		...
		{
			dsu.merge(1, v);
			if (u < v) MST.insert({u, v}); else MST.insert({v, u});
...
void build_tree(int u) {
	for (auto v: g_MST[u]) if (par[v] == -1) par[v] = u, build_tree(v);
}
...
int Ares_KN() // main
{
	...
	// count all 1-degree vertices connecting to 1
	int cnt = 0;
	for (auto v: g[1]) if (deg[v] == 1) ++cnt;
	if (cnt > k) return !printf("NO");

	// make MST
	for (auto v: g[1]) if (deg[v] == 1) 
	{
		dsu.merge(1, v);
		MST.insert({1, v});
		g_MST[1].push_back(v), g_MST[v].push_back(1);
	}
	else if (dsu.find(1) == dsu.find(v)) continue;
	else
	{
		dsu.merge(1, v);
		MST.insert({1, v});
	...
	vector<pii> res;
	for (auto v: g[1]) if (!chose[v])
	{
		// replace with 1-v
		int p = par[v];
		if (p == 1) continue;
		if (v < p) MST.erase({v, p}); else MST.erase({p, v});
		MST.insert({1, v});
		par[v] = 1;
		++cnt;
		if (cnt == k) break;
