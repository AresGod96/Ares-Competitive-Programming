/* 
	...
	- Problem: CF ROUND 1433 G - Reducing Delivery Cost
...
int Ares_KN() // main
{
	...
	int res = INF;
	for (auto E: edges) {
		int u = E.fi.fi, v = E.fi.se, w = E.se;
		int cost = 0;
		// make edge u-v zero cost
		for (auto route: routes) {
			int s = route.fi, f = route.se;
			cost += min({d[s][u] + d[v][f], d[s][v] + d[u][f], d[s][f]});
		}
		res = min(res, cost);
	}
	printf("%d\n", res);
