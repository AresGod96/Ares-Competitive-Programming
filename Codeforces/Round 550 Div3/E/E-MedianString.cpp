/* 
	...
	- Problem: CF ROUND 1144 E - Median String
	- Problem link: https://codeforces.com/contest/1144/problem/E
*/
...
int Ares_KN() // main
{
	...
	vector<int> coef(n);
	REP(i, n) coef[i] = s[i] + t[i] - 2 * 'a';

	FORD(i, n - 1, 0)
		if (i && (coef[i - 1] % 2))
		{
			if (coef[i] >= 26) ++coef[i - 1], coef[i] -= 26;
			else --coef[i - 1], coef[i] += 26;
		}

	//for (auto x: coef) cerr << x << '\n';
	for (auto x: coef) printf("%c", x / 2 + 'a');
