/* 
	...
	- Problem: CF ROUND 1141 C - Polycarp restores Permutation
...
int Ares_KN() // main
{
	...
	REP(i, n - 1) 
	{
		...
		mmax = min(mmax, ll(n) - sum);
		mmin = max(mmin, 1LL - sum);
	}
	if (S.size() < n - 1) return !printf("-1");
	if (mmin > mmax) return !printf("-1");
