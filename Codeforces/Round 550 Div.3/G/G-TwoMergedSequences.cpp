/* 
	...
	- Problem: CF ROUND 1144 G - Two Merged Sequences
	- Problem link: https://codeforces.com/contest/1144/problem/G
*/
...
int Ares_KN() // main
{
	...
	REP(i, n)
	{
		...
		{
			if (x <= a[inc.back()] && x >= a[dec.back()]) return !printf("NO");
			if (x <= a[inc.back()]) dec.push_back(i);
			else if (x >= a[dec.back()]) inc.push_back(i);
			else if (x > a[inc.back()] && x < a[dec.back()])
			{
				if (i == n - 1)
				{
					inc.push_back(i);
				...
				int y = a[i + 1];
				if (y > x) inc.push_back(i); else dec.push_back(i);
			}
		}
		else if (inc.empty())
		{
			if (dec.empty()  || x < a[dec.back()])
			{
				if (i == n - 1)
				{
					inc.push_back(i);
	...
	for (auto x: inc) a[x] = 0;
	for (auto x: dec) a[x] = 1;
	puts("YES");
