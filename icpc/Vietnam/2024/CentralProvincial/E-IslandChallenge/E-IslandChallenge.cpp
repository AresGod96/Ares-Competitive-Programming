/* 
	...
	- Problem: ICPC Vietnam Central Provincial 2024 - E - Island Challenge
	- Problem link: https://oj.vnoi.info/problem/icpc24_mt_e
*/
#define LOCAL
#undef LOCAL
...
string grid[MAXN];
int d[MAXN][MAXN];  // d[i][j]: dist between i-th shrine and j-th shrine
int dist[MAXN][MAXN];
int dx[4] = {0, -1, 0, 1};
int dy[4] = {-1, 0, 1, 0};
int dp[MAXN][1 << 16];
...
int Ares_KN() // main
{
	...
	cin >> r >> c >> n;
	vector<pii> shrines;
	// let the starting point (0, 0) be the first shrine for easy computation
	shrines.emplace_back(0, 0);
	REP(i, r) {
		cin >> grid[i];
		REP(j, c) if (grid[i][j] == 'S') {
			shrines.emplace_back(i, j);
		}
	}

	// calculate distance between pairwise shrines using BFS
	REP(i, shrines.size()) {
		pii cur_shrine = shrines[i];

		// initialize
		REP(j, shrines.size()) d[i][j] = INF;
		d[i][i] = 0;
		REP(u, r)
			REP(v, c) {
				dist[u][v] = INF;
			}
		dist[cur_shrine.fi][cur_shrine.se] = 0;
		...
		while (!Q.empty()) {
			...
			for (int j = 0; j < 4; ++j) {
				int uu = u + dx[j], vv = v + dy[j];
				if (uu >= 0 && vv >= 0 && uu < r && vv < c && grid[uu][vv] != '#' && dist[uu][vv] > dist[u][v] + 1) {
					dist[uu][vv] = dist[u][v] + 1;
					Q.push({uu, vv});
				}
			}
		}
		REP(j, shrines.size()) d[i][j] = dist[shrines[j].fi][shrines[j].se];
	}
	...
	REP(mask, 1 << n) {
		REP(i, n) if (getbit(mask, i)) {
			int prev = offbit(mask, i);
			REP(j, n) if (i != j && dp[j][prev] < INF) {
				dp[i][mask] = min(dp[i][mask], dp[j][prev] + d[i][j]);
			}
			if (mask == (1 << n) - 1) res = min(res, dp[i][mask]);
		}
	}
	printf("%d\n", res == INF ? -1 : res);
	return 0;
