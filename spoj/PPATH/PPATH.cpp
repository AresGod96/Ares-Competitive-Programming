...
int Ares_KN() // main
{
	...
	REP(u, n)
		...
		{
			int diff = 0;
			string x = toStr(prime[u]), y = toStr(prime[v]);
			REP(i, 4) diff += x[i] != y[i];
			if (diff == 1)
			{
				g[u].push_back(v);
				g[v].push_back(u);
	...
	while (nTest--)
	{
		int st, en;
		cin >> st >> en;
		st = lower_bound(begin(prime), end(prime), st) - begin(prime);
		en = lower_bound(begin(prime), end(prime), en) - begin(prime);
