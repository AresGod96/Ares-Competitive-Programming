/* 
	...
	- Problem: SPOJ - QTREE2 - Query on a tree II
    ...
    while (nTest--) {
        ...
        while (cin >> cmd) {
            ...
            if (cmd == "DIST") {
                ...
                int dist_u = lca.depth[u] - lca.depth[lca_uv];
                if (k <= dist_u) {
                    printf("%d\n", lca.get_kth_parent(u, k - 1));
                }
                else {
                    int dist_v = lca.depth[v] - lca.depth[lca_uv];
                    k -= dist_u + 1;
                    printf("%d\n", lca.get_kth_parent(v, dist_v - k));
                }
            }
        }
    }
