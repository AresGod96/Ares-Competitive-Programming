/* 
	...
	- Problem: ACM JUST Programming Contest 2017 4.0 - C - Large summation
	- Problem link: https://codeforces.com/gym/101532/problem/C
*/
...
int Ares_KN() // main
{
	...
	while (nTest--)
	{
		...
		REP(i, n)
		{
			...
			S.erase(it);
			it = S.lower_bound(INF - a[i]);
			if (it != S.begin()) --it;
			int res = (a[i] + *it) % MOD;
			it = S.lower_bound(INF * 2 - a[i]);
			if (it != S.begin()) --it;
			res = max(res, (a[i] + *it) % MOD);
			printf("%d ", res);
			S.insert(a[i]);
		}
		puts("");
