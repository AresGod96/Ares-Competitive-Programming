...
ll count_subarray(const vector<int>& a, int value) {
	ordered_multiset<ll> prefix_set;
	int n = a.size();
	vector<ll> prefix(n + 1, 0);
	REP(i, n) prefix[i] = a[i] + (i ? prefix[i - 1] : 0);

	prefix_set.insert(prefix[0]);
	ll res = a[0] >= value;
	FOR(i, 1, n - 1) {
		// count number of keys less than or equal to (prefix[i]-value)
		res += prefix_set.order_of_key(prefix[i] - value + 1);
		prefix_set.insert(prefix[i]);
