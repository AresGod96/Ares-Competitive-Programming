/* 
	...
	- Problem: USACO 2008 November Bronze - Going Once, Going Twice, Gone!
	- Problem link: https://www.acmicpc.net/problem/1246
*/
#define LOCAL
#undef LOCAL
...
int Ares_KN() // main
{
	int n, m;
	cin >> m >> n;
	vector<int> farmers(n);
	REP(i, n) cin >> farmers[i];
	sort(begin(farmers), end(farmers));

	pii res = {};
	REP(i, n)
		if (res.fi < farmers[i] * min(m, n - i)) {
			res = {farmers[i] * min(m, n - i), farmers[i]};
		}
	printf("%d %d\n", res.se, res.fi);
	return 0;
}
