/* 
	...
	- Problem: VOI 2014 Day 1 - LCS2X
	- Problem link: https://oj.vnoi.info/problem/lcs2x
	- Solution: O(N^3) using dynamic programming, still get ACcepted!?
*/
#define LOCAL
#undef LOCAL
	...
	while (nTest--) {
		...
		REP(i, n) {
			...
			REP(j, m) if (a[i] == b[j]) {
				...
				if (b[j] > 0) {
					REP(k, j)
						if (b[k] * 2 <= b[j]) dp[j] = max(dp[j], dp[k] + 1);
				}
				else {
					++cnt_zeroB;
					if (last_zeroB < j) dp[j] = min({dp[last_zeroB] + 1, cnt_zeroA, cnt_zeroB});
					last_zeroB = j;
				}
				res = max(res, dp[j]);
			}
