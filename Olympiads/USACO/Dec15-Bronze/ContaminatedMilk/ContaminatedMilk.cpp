/* 
	...
	- Problem: USACO 2015 December Bronze - Contaminated Milk
	- Problem link: http://www.usaco.org/index.php?page=viewproblem2&cpid=569
	- Problem link: https://www.acmicpc.net/problem/11972
*/
#define LOCAL
#undef LOCAL
...
int Ares_KN() // main
{
	int nPerson, nMilk, event, nSick;
	cin >> nPerson >> nMilk >> event >> nSick;
	FOR(i, 1, nPerson)
		FOR(j, 1, nMilk) drank[i][j] = INF;

	while (event--) {
		int id, milk, t;
		cin >> id >> milk >> t;
		drank[id][milk] = min(drank[id][milk], t);
	}
	REP(i, nSick) {
		int id, t;
		cin >> id >> t;
	...
	FOR(i, 1, nMilk)
		if (!good[i]) {
			// assume i is bad milk
			int cnt = 0;
			FOR(j, 1, nPerson) cnt += drank[j][i] != INF;
			res = max(res, cnt);
		}
	printf("%d\n", res);
