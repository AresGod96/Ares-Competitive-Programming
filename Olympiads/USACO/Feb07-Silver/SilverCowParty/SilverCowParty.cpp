/* 
	...
	- Problem: USACO 2007 February Silver - Silver Cow Party
...
int d[MAXN], d_rev[MAXN];
vector<pii> g[MAXN];
vector<pii> g_rev[MAXN];
int n;

void dijkstra(int s) {
	FOR(u, 1, n) d[u] = INF;
	FOR(u, 1, n) d_rev[u] = INF;
	...
	// Djikstra on reversed path
	Q.push({0, s});
	while (!Q.empty()) {
		int u = Q.top().se;
		Q.pop();
		for (auto [v, w]: g_rev[u])
			if (d_rev[v] > d_rev[u] + w) {
				d_rev[v] = d_rev[u] + w;
				Q.push({-d_rev[v], v});
			}
	}
