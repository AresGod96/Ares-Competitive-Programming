/* 
	...
	- Problem: CSES 2220 - Counting Numbers
...
ll calc(int pos, int prev, bool tight, bool started) {
    if (pos == num.size()) return 1;
    if (!tight && dp[pos][prev][started] != -1) {
        return dp[pos][prev][started];
    }
    ...
    FOR(digit, 0, limit) {
        int ntight = tight && (digit == num[pos] - '0');
        // started: already have a non-zero digit or not
        if (started && digit == prev) continue;
        res += calc(pos + 1, digit, ntight, started || digit > 0);
    }
    if (!tight) dp[pos][prev][started] = res;
    return res;
}
