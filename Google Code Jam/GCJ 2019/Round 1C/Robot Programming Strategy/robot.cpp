/* 
	...
	- Problem: GCJ 2019 Round 1C - Robot Programming Strategy
...
int Ares_KN() // main
{
	...
	while (nTest--)
	{
		...
		while (1)
		{
			...
			REP(i, n) if (!flag[i])
			{
				char c = s[i][cur % s[i].size()];
				if (win(tmp, c) == 1) flag[i] = true;
			}
			++cur;
			if (cur == 500) break;
		}
		if (bad || res.size() > 500) puts("IMPOSSIBLE");
		else printf("%s\n", res.c_str());
	}
