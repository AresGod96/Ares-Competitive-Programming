/* 
	...
	- Problem: Baekjoon OJ - 2261 - 가장 가까운 두 점
	- Problem link: https://www.acmicpc.net/problem/2261

	- Solution: divde and conquer
	- Complexity: O(NlogN)
...
int closest_pair(int l, int r) {
	...
	int dl = closest_pair(l, mid);
	int dr = closest_pair(mid, r);
	int res = min(dl, dr);

	int sz = 0;
	for (int i = l; i < r; ++i)
		if ((points[i].x - mid_x) * (points[i].x - mid_x) <= res) {
			strip[sz++] = points[i];
		}

	sort(strip.begin(), strip.begin() + sz, [](const Point &a, const Point &b) {
		return a.y < b.y;
	});
...
int Ares_KN() // main
{
	...
	sort(begin(points), end(points), [](const Point &a, Point &b) {
		return a.x < b.x || (a.x == b.x && a.y < b.y);
	});

	int res = closest_pair(0, n);
	printf("%d\n", res);
