/* 
	...
	- Problem: Baekjoon OJ - 13547 - 수열과 쿼리 5
	- Problem link: https://www.acmicpc.net/problem/13547

	- Tags
		- merge sort tree (too slow)
		- Mo's algorithm
*/
#define LOCAL
#undef LOCAL
...
struct Query {
	...
	bool operator<(const Query& other) const { 
		...
		return (l / BLOCK_SZ % 2 == 0) ? (r < other.r) : (r > other.r);
	}
};
...
int Ares_KN() // main
{
	...
	vector<int> res(q);
	int cur_l = queries[0].l - 1, cur_r = queries[0].r - 1;
	for (int i = cur_l; i <= cur_r; ++i) add(a[i]);
