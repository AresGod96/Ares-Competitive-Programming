/* 
	...
	- Problem: Baekjoon OJ - 1081 - 합
	- Problem link: https://www.acmicpc.net/problem/1081
*/
#define LOCAL
#undef LOCAL
...
ll calc(ll n, int len) {
	...
	int fdigit = n / p10[len - 1];
	ll res = (fdigit * (fdigit - 1)) / 2 * p10[len - 1];
	res += fdigit * ((n % p10[len - 1]) + 1);
	res += fdigit * f[len - 1];
	res += calc(n % p10[len - 1], len - 1);
	return res;
}
...
int Ares_KN() // main
{
	...
	FOR(i, 1, 16) {
		p10[i] = p10[i - 1] * 10;
		f[i] = i == 1 ? 45 : (p10[i - 1] * 45 + f[i - 1] * 10);
	}
	...
	printf("%lld\n", calc(b, get_len(b)) - calc(max(0LL, a - 1), get_len(a - 1)));
