/* 
	...
	- Problem: Baekjoon OJ - 1517 - 버블 소트
...
int Ares_KN() // main
{
	...
	auto compress = [&](const vector<int> arr) {
		...
		for (int val: arr) {
			compressed_arr.emplace_back(lower_bound(begin(sorted_arr), end(sorted_arr), val) - begin(sorted_arr) + 1);
		}
