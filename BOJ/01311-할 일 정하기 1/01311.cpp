/* 
	...
	- Problem: Baekjoon OJ - 1311 - 할 일 정하기 1
	- Problem link: https://www.acmicpc.net/problem/1311
*/
#define LOCAL
#undef LOCAL
...
int Ares_KN() // main
{
	...
	FOR(mask, 1, (1 << n) - 1) state[cntbit(mask) - 1].push_back(mask);

	// dp each column
	REP(i, n) dp[0][1 << i] = cost[i][0];
	FOR(i, 1, n - 1) {
		for (auto mask: state[i]) {
			dp[i][mask] = INF;
			REP(j, n) if ((mask >> j) & 1) {
				int p_mask = offbit(mask, j);
				dp[i][mask] = min(dp[i][mask], dp[i - 1][p_mask] + cost[j][i]);
			}
		}
	}
	printf("%d\n", dp[n - 1][(1 << n) - 1]);
