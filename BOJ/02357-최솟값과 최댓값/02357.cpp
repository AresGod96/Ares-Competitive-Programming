/* 
	...
	- Problem: Baekjoon OJ - 2357 - 최솟값과 최댓값
...
// - Range Minimum Query
// - Given a static array A with N elements and Q queries
// - With each query L R output the min/max value in A[L]..A[R]
// - Complexity: initiation O(NlogN), query O(logN)
// Range Min/Max Query
struct RangeQuery {
	...
	void initRMQ(vector<int> a) {
		...
		p.resize(n + 1);
		for (int i = 1; i <= n; ++i) rmin[i].resize(25), rmax[i].resize(25);
...
int Ares_KN() // main
{
	...
	while (q--) {
		...
		cin >> l >> r;
		--l, --r;
		printf("%d %d\n", rmq.get_min(l, r), rmq.get_max(l, r));
	}
