/* 
	...
	- Problem: PreVNOI 2013 Day 2 - Problem 2 - Queens
	- Problem link: https://oj.vnoi.info/problem/queennb
*/
#define LOCAL
#undef LOCAL
...
string s[MAXN];
vector<int> row[MAXN], col[MAXN], d1[MAXN + offset], d2[MAXN];
...
int Ares_KN() // main
{
	...
	REP(i, n) {
		...
		REP(j, m)
			if (s[i][j] == '#') {
				...
				d1[i - j + offset].emplace_back(i);
				d2[i + j].emplace_back(i);
			}
	}
	...
	REP(i, n) {
		REP(j, m) {
			...
			int l, r;
			// row
			auto it = lower_bound(begin(row[i]), end(row[i]), j);
			if (it != row[i].begin())
				l = *(it - 1) + 1;
			else
				l = 0;
			...
			// col
			it = lower_bound(begin(col[j]), end(col[j]), i);
			if (it != col[j].begin())
				l = *(it - 1) + 1;
			else
				l = 0;
			...
			// upper main diag
			it = lower_bound(begin(d1[i - j + offset]), end(d1[i - j + offset]), i);
			if (it != d1[i - j + offset].begin()) {
				--it;
				res += i - *it;
			}
			else {
			...
			// upper sub diag
			it = lower_bound(begin(d2[i + j]), end(d2[i + j]), i);
			if (it != d2[i + j].begin()) {
				--it;
				res += i - *it;
			}
			else {
				int tmp = max(0, i + j - (m - 1));
				res += i - tmp + 1;
			}
			// lower sub diag
			it = lower_bound(begin(d2[i + j]), end(d2[i + j]), i);
